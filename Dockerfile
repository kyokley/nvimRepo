...
RUN sed -i "s#let g:python3_dir.*#let g:python3_dir = '/usr/local/bin/'#" /root/.config/nvim/configs/plugins.vim && \
        ...
        sed -i 's!autocmd BufEnter \* let \&titlestring = "dvim " \. expand("%:p")!autocmd BufEnter * let \&titlestring = "dvim (" . g:git_root . ") " . expand("%:p")[len("/files") + 1:]!' /root/.config/nvim/configs/autocommands.vim && \
